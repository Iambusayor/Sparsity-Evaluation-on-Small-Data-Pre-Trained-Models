...
from ner_utils import convert_examples_to_features, read_examples_from_file_noise
from ner_utils import get_labels
from ner_utils import read_examples_from_file
from ner_utils import write_csv
from ner_utils import print_size_of_model
import wandb
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, prefix=""):
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()
    eval_dataset = load_and_cache_examples_noise(
...
def load_and_cache_examples_noise(args, tokenizer, labels, pad_token_label_id, mode):
    if args.local_rank not in [-1, 0] and not evaluate:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        logger.info("Creating features from dataset file at %s", args.data_dir)
        examples = read_examples_from_file_noise(args.data_dir, mode)
        features = convert_examples_to_features(
            examples,
            labels,
            args.max_seq_length,
...
def main():
    ...
    parser.add_argument(
        "--seed", type=int, default=999, help="random seed for initialization"
    )
    ...
    parser.add_argument(
        "--csv_file", type=str, default="results.csv", help="for logging results."
    )
    ...
    wandb.init(project=args.wand_project_name, sync_tensorboard=True, entity =args.wandb_entity_name, name = args.wandb_name)
    if (
